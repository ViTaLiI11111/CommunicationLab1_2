...
logger = logging.getLogger(__name__) # Отримуємо об'єкт логера
...
class QuestionData:
    def __init__(self):
        ...
        # Коректно визначаємо корінь проекту. Path(__file__) - шлях до поточного файлу.
                ...
                if isinstance(data, list): # Перевіряємо, чи кореневий елемент - список
                    for item in data:
                        ...
                        if isinstance(item, dict) and "question" in item and "answers" in item:
                            ...
                            logger.warning(f"Invalid data format for an item in {filename}. Expected dict with 'question' and 'answers'. Skipping entry: {item}") # Логуємо попередження
                else:
                    logger.warning(f"Invalid root data format in {filename}. Expected a list of questions. Skipping file.") # Логуємо попередження
